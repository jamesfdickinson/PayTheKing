...
function Settings() {
    ...
    this.settings = {
        'name': 'Player' + Math.floor((Math.random() * 10000) + 1),//todo: set random name
        'email': null,
    ...
    //update session and perm settings
    this.add = function (key, value) {
        this.settings[key] = value;
        this.save();//could do a save when done manually or save when app is closing
    }
    this.clear = function () {
        this.settings = {};
        this.save();
    ...
    this.reset = function () {
        ...
        this.settings = {
            'name': 'Player' + Math.floor((Math.random() * 10000) + 1),//set random name
            'deviceId': deviceId,
            'email': null,
            'avatar': 'images/Avatars/default-avatar.png',
            'facebookid': 0,
    ...
    this.isLoggedIn = function () {
        if (this.settings.facebooktoken) return true;
        return false;
    }
    this.getFaceBookImage = function (facebookId) {
    ...
    this.get_gravatar = function (email, size) {
        if (!email) return 'https://www.gravatar.com/avatar/HASH';
        ...
        var size = size || 80;
        return 'https://www.gravatar.com/avatar/' + MD5(email) + '.jpg?s=' + size + 'r=pg&d=wavatar';
    }
    ...
    this.load = function () {
        ...
        //get settings
        //read from static in memory as pulling from storage is not working some times on same page.
        ...
        //load defaults
        if (this.settings["name"] === undefined) this.settings["name"] = 'No Name';
        if (this.settings["email"] === undefined) this.settings["email"] = null;
        if (this.settings["id"] === undefined) this.settings["id"] = null;
        if (this.settings["rating"] === undefined) this.settings["rating"] = 1000;
        if (this.settings["rank"] === undefined) this.settings["rank"] = 'Bronze';
        if (this.settings["useCanvas"] === undefined) this.settings["useCanvas"] = false;
        if (this.settings["device"] === undefined) {
            ...
            if (typeof Config !== 'undefined' && Config && Config.OS) {
                os = Config.OS;
            }
        ...
        //this.settings["level"] = this.getLevel();
        ////this.settings["rank"] = "Level " + this.getLevel();
